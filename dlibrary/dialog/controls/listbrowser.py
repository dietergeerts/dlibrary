from dlibrary.dialog.control import AlignModeEnum, AbstractDataContext, TextAlignEnum, AbstractListControl
from dlibrary.utility import converter
import vs


class ControlTypeEnum(object):
    STATIC = 1; RADIO_ICON = 2; TOGGLE = 3; TOGGLE_ICON = 4; STATIC_ICON = 5; NUMBER = 6; MULTI_ICON = 7
...
class DisplayTypeEnum(object):
    TEXT_ONLY = 0; ICON_ONLY = 1; TEXT_AND_ICON = 3
...
class Column(object):
    ...
    @property
    def control_type(self): return self.__control_type

    @property
    def display_type(self): return self.__display_type

    @property
    def text_align(self): return self.__text_align

    @property
    def data_value(self): return self.__data_value
...
class ListBrowser(AbstractListControl):
    ...
    def _on_item_changed(self, index: int, item: object, value_index: int):
        ...
        vs.SetLBItemInfo(self._dialog_id, self.control_id, self.__get_control_index(index), column_index,
                         self._field(item, self.__columns[column_index].data_value).value, -1)

    def _add_control_item(self, index: int, item: object):
        if self.__index: self.__update_index_column(index, +1)
        ...
        for column_index, column in enumerate(self.__columns):
            vs.SetLBItemInfo(
                ...
                (index + 1) if (self.__index and column_index == 0) else self._field(item, column.data_value).value, -1)
    ...
    def __update_index_column(self, changed_index, modifier):
        for control_index in range(0, vs.GetNumLBItems(self._dialog_id, self.control_id)):
            ...
            if item_index >= changed_index: vs.SetLBItemInfo(
    ...
    def __get_control_index(self, item_index: int) -> int:
        for control_index in range(0, vs.GetNumLBItems(self._dialog_id, self.control_id)):
            item_info = vs.GetLBItemInfo(self._dialog_id, self.control_id, control_index, 0)
            if int(item_info[1]) - 1 == item_index: return control_index
    ...
    def __on_single_click(self, data: int, row_index: int, column_index: int):
        if self.__is_drag_drop_event(data): self.__on_drag_drop(data, row_index, column_index)
        else: self.__on_selection(data, row_index, column_index)
    ...
    def __on_column_header_click(self, data: int, row_index: int, column_index: int):
        ...
        if self.__index: vs.EnableLBDragAndDrop(

...
class EditText(AbstractFieldControl):
    @classmethod
    def can_align(cls, layout: int) -> bool: return layout == LayoutEnum.VERTICAL
    ...
    def __init__(self, dialog_id: int, control_id: int, help_text: str, data_parent: AbstractDataContext,
                 data_context: str, data_value: str, data_items: str, width: int):
    ...
    def _get_control_value(self): return vs.GetItemText(self._dialog_id, self.control_id)

    def _select_control_value(self): vs.SelectEditText(self._dialog_id, self.control_id)

    def _on_control_event(self, data: int): self._value = self._get_control_value()

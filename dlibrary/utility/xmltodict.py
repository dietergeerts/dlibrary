...
def load(path: str, list_elements: set, defaults: dict):
    try:
        with open(path) as file:
            return __set_defaults(__correct(xmltodict.parse(file.read()), list_elements), defaults)
...
def load_or_create_if_not_found(path: str, list_elements: set, defaults: dict):
    try:
        if os.path.isfile(path):
            return load(path, list_elements, defaults)
        else:
            return __set_defaults(dict(), defaults)
    except (FileNotFoundError, PermissionError, OSError):
        raise
...
def save(elements: dict, path: str):
    try:
        with open(path, 'w') as file:
            xmltodict.unparse(elements, file)
    except (FileNotFoundError, PermissionError, OSError):
        raise
    # TODO: Check contents of file with an xml schema?
...
def __correct(elements: dict, list_elements: set) -> dict:
    for name in (k for k in elements.keys() if k[:1] != '@' and k[:1] != '#'):
        ...
        if elements[name] is None:
            elements[name] = {}
        # If an element only has an inner text, we don't get a dict, but a str instead!
        if isinstance(elements[name], str):
            elements[name] = {'#text': elements[name]}
        ...
        if isinstance(elements[name], OrderedDict) and name in list_elements:
        ...
        if isinstance(elements[name], list):
            for element in elements[name]:
                __correct(element, list_elements)
        else:
            __correct(elements[name], list_elements)
    return elements
    ...
    for name in defaults.keys():
        ...
        # The default value could be a list, with a possible dict to set defaults.
            ...
            if name not in elements:
                ...
                for element in elements[name]:
                    __set_defaults(element, defaults[name][0])
